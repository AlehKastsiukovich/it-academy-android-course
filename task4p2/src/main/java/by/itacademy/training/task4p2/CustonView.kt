...
class CustomView @JvmOverloads constructor(
    ...
    private fun shuffleAllColor() {
        colorArc0 = util.getRandomColor()
        colorArc1 = util.getRandomColor()
        colorArc2 = util.getRandomColor()
        colorArc3 = util.getRandomColor()
    }

    private fun checkWhereWasEventTouch(x: Float, y: Float) {
        if (util.checkIfPointInCircle(x, y, centerX.toFloat(), centerY.toFloat(), RADIUS_INNER)) {
            shuffleAllColor()
            listener?.onTouchColorMixer(x, y, null)
            invalidate()
        } else if (util.checkIfPointInCircle(x, y, centerX.toFloat(), centerY.toFloat(), RADIUS_OUTER)) {
